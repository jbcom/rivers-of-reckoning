...
def shop_menu(stdscr, player):
    while True:
        ...
        for idx, item in enumerate(SHOP_ITEMS):
            ...
            stdscr.addstr(5 + idx, 4, f"{idx+1}. {item['name']} ({item['cost']} gold): {item['desc']}{owned}")
            ...
            if key == ord(str(idx+1)):
                if player.gold >= item['cost']:
                    ...
                    if item['effect'] == 'potion':
                        ...
                        stdscr.addstr(5 + len(SHOP_ITEMS) + 2, 4, f"Sword upgraded to level {player.sword_level}!")
                    elif item['effect'] == 'shield':
                        player.shield_level += 1
                        stdscr.addstr(5 + len(SHOP_ITEMS) + 2, 4, f"Shield upgraded to level {player.shield_level}!")
                    elif item['effect'] == 'boots':
                        player.boots_level += 1
                        stdscr.addstr(5 + len(SHOP_ITEMS) + 2, 4, f"Boots upgraded to level {player.boots_level}!")
